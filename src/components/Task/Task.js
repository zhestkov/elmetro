  ...
  render() {
    ...
    return (
      <div className="container">
        ...
        <p>
          ...
          {editTextMode ? (
            <input
              ...
              value={text}
              onBlur={() => this.toggleTextMode()}
            />
          ) : (
            <Fragment>
              <span className={this.getClasses("text")}>{text}</span>
              <span className="edit-icon" onClick={this.toggleTextMode}>
                &#x270E;
              </span>
            </Fragment>
          )}
        ...
        <p>
          <span className="header">Instruction:</span>
          {editInstructionMode ? (
            <input
              type="text"
              className="form-control"
              ...
              value={instruction}
              onBlur={() => this.toggleInstructionMode()}
            />
          ) : (
            <Fragment>
              <span className={this.getClasses("instruction")}>
                {instruction}
              </span>
              <span className="edit-icon" onClick={this.toggleInstructionMode}>
                &#x270E;
              </span>
            </Fragment>
          )}
        ...
        <button className="btn btn-success" onClick={this.addFragment}>
          Add fragment
        </button>
        ...
        <div className="file-upload">
          ...
          <label className="file-upload btn btn-primary" for="file">
            Add audio
          </label>
        </div>
        {audio[0] &&
          audio[0].status === 200 && (
            <audio
              src={`http://localhost:8080/tasks/${
                taskModel.sampleTask.id
              }/downloadAudio`}
              controls
            >
